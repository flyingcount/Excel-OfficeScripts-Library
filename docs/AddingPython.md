...
**Time series only:** paste `source/python-in-excel/init/TimeSeries.py` instead. That file is Excel defaults plus time series functions (`expsmooth`, `stl`, `stl_plot`, `resid_analysis`, `acf_ljungbox`, `acf_pacf`, `adf_test`, `arima_order`, `arima_estimate`, `baseline_forecast`, `zscore_replace`, `date_features`, and any later series tools). It does not include the rest of the library.
